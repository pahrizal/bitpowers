...
#
# Sync this bitpowers checkout → prime-radiant-inc/openai-codex-plugins.
...
# plugins/bitpowers/ when absent, then copies the tracked plugin files from
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  SYNC_BRANCH="bootstrap/bitpowers-${UPSTREAM_SHORT}-${TIMESTAMP}"
else
  SYNC_BRANCH="sync/bitpowers-${UPSTREAM_SHORT}-${TIMESTAMP}"
fi
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  echo "Mode:     BOOTSTRAP (creating plugins/bitpowers/ when absent)"
fi
echo ""
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  COMMIT_TITLE="bootstrap bitpowers v$UPSTREAM_VERSION from upstream main @ $UPSTREAM_SHORT"
  PR_BODY="Initial bootstrap of the bitpowers plugin from upstream \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).

Creates \`plugins/bitpowers/\` by copying the tracked plugin files from upstream, including \`.codex-plugin/plugin.json\` and \`assets/\`.

Run via: \`scripts/sync-to-codex-plugin.sh --bootstrap\`
Upstream commit: https://github.com/pahrizal/bitpowers/commit/$UPSTREAM_SHA
...
This is a one-time bootstrap. Subsequent syncs will be normal (non-bootstrap) runs using the same tracked upstream plugin files."
  ...
  COMMIT_TITLE="sync bitpowers v$UPSTREAM_VERSION from upstream main @ $UPSTREAM_SHORT"
  PR_BODY="Automated sync from bitpowers upstream \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).
...
Upstream: https://github.com/pahrizal/bitpowers/commit/$UPSTREAM_SHA
Branch:   $SYNC_BRANCH"
