...
Bitpowers is opinionated about worktree management — specific paths (`.worktrees/<branch>`), specific commands (`git worktree add`), specific cleanup (`git worktree remove`). Meanwhile, Claude Code, Codex App, Gemini CLI, and Cursor all provide native worktree support with their own paths, lifecycle management, and cleanup.
...
For harnesses without native support (Codex CLI, OpenCode, Copilot standalone), bitpowers fills a real gap. The skill shouldn't go away — it should get out of the way when native support exists.
...
- `.bitpowers-session.json` metadata tracking (interesting PR #997 idea, not needed for v1)
...
Whoever creates the worktree owns its cleanup. If the harness created it, bitpowers doesn't touch it. If bitpowers created it (via git fallback), bitpowers cleans it up. The heuristic: if the worktree lives under `.worktrees/` or `~/.config/bitpowers/worktrees/`, bitpowers owns it. Anything else (`.claude/worktrees/`, `~/.codex/worktrees/`, `.gemini/worktrees/`) belongs to the harness.
...
2. Check for existing `~/.config/bitpowers/worktrees/<project>/` directory — if found, use it (backward compatibility with legacy global path).
...
No interactive directory selection prompt. The global path (`~/.config/bitpowers/worktrees/`) is no longer offered as a choice to new users, but existing worktrees at that location are detected and used for backward compatibility.
...
git worktree remove "$WORKTREE_PATH"  # only if bitpowers owns it
...
**Option 4 (Discard):** Require typed "discard" confirmation. Then remove worktree (if bitpowers owns it), force-delete branch.
...
if worktree path is under .worktrees/ or ~/.config/bitpowers/worktrees/:
    # Bitpowers created it — we own cleanup
...
The `.worktrees/` or `~/.config/bitpowers/worktrees/` = ours, anything else = hands off` heuristic works for every current harness. If a future harness adopts `.worktrees/` as its convention, we'd have a false positive (bitpowers tries to clean up a harness-owned worktree). Similarly, if a user manually runs `git worktree add .worktrees/experiment` without bitpowers, we'd incorrectly claim ownership. Both are low risk — every harness uses branded paths, and manual `.worktrees/` creation is unlikely — but worth noting.
