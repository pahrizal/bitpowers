...
> **For agentic workers:** REQUIRED SUB-SKILL: Use bitpowers:subagent-driven-development (recommended) or bitpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make bitpowers defer to native harness worktree systems when available, fall back to manual git worktrees when not, and fix three known finishing bugs.
...
**Spec:** `docs/bitpowers/specs/2026-04-06-worktree-rototill-design.md`
...
2. **Check for existing global directory:**
   ...
   ls -d ~/.config/bitpowers/worktrees/$project 2>/dev/null
   ```
...
Global directories (`~/.config/bitpowers/worktrees/`) need no verification.
...
# For global: path="~/.config/bitpowers/worktrees/$project/$BRANCH_NAME"
...
**If worktree path is under `.worktrees/` or `~/.config/bitpowers/worktrees/`:** Bitpowers created this worktree — we own cleanup.
...
- **Fix:** Only clean up worktrees under `.worktrees/` or `~/.config/bitpowers/worktrees/`
...
```markdown
- **bitpowers:using-git-worktrees** - REQUIRED: Set up isolated workspace before starting
```

to:

```markdown
- **bitpowers:using-git-worktrees** - Ensures isolated workspace (creates one or verifies existing)
```
