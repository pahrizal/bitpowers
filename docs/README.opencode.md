# Bitpowers for OpenCode

Complete guide for using Bitpowers with [OpenCode.ai](https://opencode.ai).
...
**Skill Priority:** Project skills > Personal skills > Bitpowers skills
...
1. **Injects bootstrap context** via the `experimental.chat.system.transform` hook, adding bitpowers awareness to every conversation.
2. **Registers the skills directory** via the `config` hook, so OpenCode discovers all bitpowers skills without symlinks or manual config.
...
1. Check OpenCode logs: `opencode run --print-logs "hello" 2>&1 | grep -i bitpowers`
