...
> **For agentic workers:** REQUIRED SUB-SKILL: Use bitpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add full bitpowers support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/bitpowers-codex` (lines 40-74)
...
- Reference: `.codex/bitpowers-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'bitpowers' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/bitpowers-codex` (lines 212-280)
 ...
 * (personal skills override bitpowers skills).
 *
 * @param {string} skillName - Name like "bitpowers:brainstorming" or "my-skill"
 * @param {string} bitpowersDir - Path to bitpowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, bitpowersDir, personalDir) {
    // Strip bitpowers: prefix if present
    const forceBitpowers = skillName.startsWith('bitpowers:');
    const actualSkillName = forceBitpowers ? skillName.replace(/^bitpowers:/, '') : skillName;

    // Try personal skills first (unless explicitly bitpowers:)
    if (!forceBitpowers && personalDir) {
    ...
    // Try bitpowers skills
    if (bitpowersDir) {
        const bitpowersPath = path.join(bitpowersDir, actualSkillName);
        const bitpowersSkillFile = path.join(bitpowersPath, 'SKILL.md');
        if (fs.existsSync(bitpowersSkillFile)) {
            return {
                skillFile: bitpowersSkillFile,
                sourceType: 'bitpowers',
                skillPath: actualSkillName
            };
        }
    }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/bitpowers-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/bitpowers-codex` (add import at top)
...
Run: `node -c .codex/bitpowers-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/bitpowers-codex
...
**Files:**
- Modify: `.codex/bitpowers-codex` (lines 40-74)
...
Run: `.codex/bitpowers-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/bitpowers-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/bitpowers-codex` (lines 16-38, approximately)
...
Run: `.codex/bitpowers-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/bitpowers.js`
...
/**
 * Bitpowers plugin for OpenCode.ai
 *
...
const bitpowersSkillsDir = path.join(homeDir, '.config/opencode/bitpowers/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const BitpowersPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/bitpowers.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/bitpowers.js
...
**Files:**
- Modify: `.opencode/plugin/bitpowers.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "bitpowers:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > bitpowers)
...
Run: `node -c .opencode/plugin/bitpowers.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the bitpowers and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const bitpowersSkills = skillsCore.findSkillsInDir(
            bitpowersSkillsDir,
            'bitpowers',
            3
          );
          ...
          const allSkills = [...personalSkills, ...bitpowersSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install bitpowers skills to ~/.config/opencode/bitpowers/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'bitpowers:';
...
```javascript
    'session.started': async () => {
      // Read using-bitpowers skill content
      const usingBitpowersPath = skillsCore.resolveSkillPath(
        'using-bitpowers',
        bitpowersSkillsDir,
        personalSkillsDir
      );

      let usingBitpowersContent = '';
      if (usingBitpowersPath) {
        const fullContent = fs.readFileSync(usingBitpowersPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingBitpowersContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Bitpowers skills: \`bitpowers:skill-name\` (from ~/.config/opencode/bitpowers/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/bitpowers')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/bitpowers && git pull` to update bitpowers.' :
        '';
...
**Below is the full content of your 'bitpowers:using-bitpowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingBitpowersContent}
...
The plugin is included in the bitpowers repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/bitpowers/.opencode/plugin/bitpowers.js`
...
ln -s ~/.config/opencode/bitpowers/.opencode/plugin/bitpowers.js .opencode/plugin/bitpowers.js
```
...
```
use use_skill tool with skill_name: "bitpowers:brainstorming"
```
...
Personal skills override bitpowers skills with the same name.

## Updating

```bash
cd ~/.config/opencode/bitpowers
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/bitpowers/.opencode/plugin/bitpowers.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/bitpowers/skills`
...
Bitpowers works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/bitpowers-codex use-skill bitpowers:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/bitpowers-codex bootstrap | head -30`
...
ls -l .opencode/plugin/bitpowers.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/bitpowers.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/bitpowers-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/bitpowers-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/bitpowers.js` created with tools and hooks
