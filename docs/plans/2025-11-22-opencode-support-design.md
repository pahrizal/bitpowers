...
Add full bitpowers support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port bitpowers to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/bitpowers-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/bitpowers.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/bitpowers/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/bitpowers-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "bitpowers:brainstorming")')
  }),
...
1. **Inject using-bitpowers content**
   - Full content of the using-bitpowers skill
...
```javascript
// .opencode/plugin/bitpowers.js
const skillsCore = require('../../lib/skills-core');
const path = require('path');
const fs = require('fs');
const { z } = require('zod');

export const BitpowersPlugin = async ({ client, directory, $ }) => {
  const bitpowersDir = path.join(process.env.HOME, '.config/opencode/bitpowers');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingBitpowers = await readSkill('using-bitpowers');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingBitpowers}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── bitpowers-codex        # UPDATED: Use skills-core
│   ├── bitpowers-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── bitpowers.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/bitpowers-codex`
...
2. Update `.codex/bitpowers-codex` to use shared core
...
1. Create `.opencode/plugin/bitpowers.js`
