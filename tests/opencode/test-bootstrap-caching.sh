...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$BITPOWERS_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$BITPOWERS_SKILLS_DIR/using-bitpowers/SKILL.md" "$TEST_HOME/using-bitpowers.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$BITPOWERS_PLUGIN_FILE" missing
}
