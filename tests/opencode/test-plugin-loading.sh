#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the bitpowers plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/bitpowers.js"
...
skill_count=$(find "$BITPOWERS_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $BITPOWERS_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-bitpowers skill exists (critical for bootstrap)
echo "Test 3: Checking using-bitpowers skill (required for bootstrap)..."
if [ -f "$BITPOWERS_SKILLS_DIR/using-bitpowers/SKILL.md" ]; then
    echo "  [PASS] using-bitpowers skill exists"
else
    echo "  [FAIL] using-bitpowers skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$BITPOWERS_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/bitpowers/' "$BITPOWERS_PLUGIN_FILE"; then
