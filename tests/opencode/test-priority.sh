...
# 1. Create in bitpowers location (lowest priority)
mkdir -p "$BITPOWERS_SKILLS_DIR/priority-test"
cat > "$BITPOWERS_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Bitpowers version of priority test skill
---
# Priority Test Skill (Bitpowers Version)

This is the BITPOWERS version of the priority test skill.

PRIORITY_MARKER_BITPOWERS_VERSION
EOF
...
if [ -f "$BITPOWERS_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Bitpowers version exists"
else
    echo "  [FAIL] Bitpowers version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled bitpowers priority
echo ""
echo "Test 2: Documenting personal vs bitpowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled bitpowers version instead of personal version"

# Test 3: Document project vs bundled bitpowers priority
echo ""
echo "Test 3: Documenting project vs personal/bitpowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled bitpowers version instead of project version"

# Test 4: Test a non-colliding bundled bitpowers skill is still available
echo ""
echo "Test 4: Testing non-colliding bitpowers skill remains available..."

mkdir -p "$BITPOWERS_SKILLS_DIR/bitpowers-only-test"
cat > "$BITPOWERS_SKILLS_DIR/bitpowers-only-test/SKILL.md" <<'EOF'
---
name: bitpowers-only-test
description: Bitpowers-only priority test skill
---
# Bitpowers Only Test Skill

PRIORITY_MARKER_BITPOWERS_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"bitpowers-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_BITPOWERS_ONLY_VERSION" "Non-colliding bitpowers skill is still registered"
