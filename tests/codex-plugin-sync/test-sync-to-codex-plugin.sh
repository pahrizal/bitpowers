...
EOF
    ...
    cat > "$repo/assets/bitpowers-small.svg" <<'EOF'
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/bitpowers/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/bitpowers/.fixture-keep"
    cat > "$repo/plugins/bitpowers/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/bitpowers/.fixture-keep
    git -C "$repo" add plugins/bitpowers/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/bitpowers/skills/example/agents"

    cat > "$repo/plugins/bitpowers/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/bitpowers/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/bitpowers/.codex-plugin" \
        "$repo/plugins/bitpowers/.private-journal" \
        "$repo/plugins/bitpowers/assets" \
        "$repo/plugins/bitpowers/skills/example/agents" \
        "$repo/plugins/bitpowers/skills/example"

    cat > "$repo/plugins/bitpowers/.codex-plugin/plugin.json" <<EOF
{
  "name": "bitpowers",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/bitpowers/assets/bitpowers-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/bitpowers/assets/app-icon.png"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/bitpowers/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/bitpowers/.codex-plugin/plugin.json \
        plugins/bitpowers/assets/app-icon.png \
        plugins/bitpowers/assets/bitpowers-small.svg \
        plugins/bitpowers/skills/example/agents/openai.yaml \
        plugins/bitpowers/skills/example/SKILL.md \
        plugins/bitpowers/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/bitpowers/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/bitpowers/.fixture-keep"
    printf 'stale ignored leak\n' > "$repo/plugins/bitpowers/.private-journal/leak.txt"
    git -C "$repo" add plugins/bitpowers/.fixture-keep
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/bitpowers/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/bitpowers/skills/example/agents/openai.yaml"
    ...
    assert_contains "$preview_section" "assets/bitpowers-small.svg" "Preview includes SVG asset"
    ...
    assert_not_contains "$preview_output" "Assets (bitpowers-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/bitpowers-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/bitpowers/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/bitpowers" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/bitpowers-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/bitpowers'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/bitpowers-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/bitpowers-*" "Clean no-op local apply does not create sync branch in destination checkout"
