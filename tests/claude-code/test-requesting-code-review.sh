...
Use the bitpowers:requesting-code-review skill to review these changes before I merge. Follow the skill exactly: dispatch the code reviewer subagent with the template, give the subagent the SHA range, and report back what it found.
...
if [ -z "$SESSION_FILE" ] || [ ! -f "$SESSION_FILE" ]; then
    ...
    FAILED=$((FAILED + 1))
elif ! grep -q '"skill":"bitpowers:requesting-code-review"' "$SESSION_FILE"; then
